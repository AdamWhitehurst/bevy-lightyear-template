...
- [ ] Confirm no cargo build/check/test is running:
      `pgrep -af 'cargo (build|check|test|make|server|client)|rustc|rustdoc' || true`
- [ ] `cargo test -p server --test world_object_edit delete` passes
- [ ] Confirm no cargo build/check/test is running again
- [ ] `cargo test -p client --features spawn-panel --test plugin selection` passes
- [ ] Confirm no cargo build/check/test is running again
- [ ] `cargo test -p voxel_map_engine persistence::tests::empty_entities_file_is_authoritative_empty` passes

#### Manual

- [ ] Run `cargo server`, then `cargo client`; press `F4`, then `F6`.
- [ ] Select an existing replicated world object from the nearby selection state.
- [ ] Press `Delete` or click the delete button if implemented.
- [ ] Confirm the selected object disappears on the client by server replication, not local-only despawn.
- [ ] Unload and reload the chunk containing the deleted object.
- [ ] Confirm the deleted/generated object does not return.
...
- [ ] After Phase 1: a selected replicated object can be deleted authoritatively, its chunk save can be empty, and
