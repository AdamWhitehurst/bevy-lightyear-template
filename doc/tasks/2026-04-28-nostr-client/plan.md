...
Move shared profile path rules into `nostr_client`: `client_identity_dir(profile: Option<&str>) -> Result<PathBuf, String>` returns `$XDG_CONFIG_HOME/nostr` (or `~/.config/nostr`) for the default profile and `$XDG_CONFIG_HOME/nostr/profiles/<profile>` for named profiles. `crates/client/src/persistence/fs_encrypted_identity.rs` delegates to that helper. Profile names must be non-empty, start with an ASCII letter or digit, and contain only ASCII letters, digits, `.`, `_`, or `-`.
...
#### Manual
- [x] Run two native clients as `cargo client -- --nostr-identity alice` and `cargo client -- --nostr-identity bob`; each profile loads or creates an independent encrypted identity.
