...
pub use systems::{
    cleanup_hitbox_entities, process_hitbox_hits, process_projectile_hits,
    update_hitbox_positions,
};

pub(crate) use effects::apply_on_hit_effects;
...
Note: remove `AbilityPlugin` from the `pub use types::` line — it comes from `plugin::`. The list
above must exactly cover what `lib.rs` currently imports from `crate::ability` (lines 17–24 of
`lib.rs`) plus what `hit_detection/` imports.
...
4. In `SharedGameplayPlugin::build`, change:
   ```rust
